...
#'
#' El selector de shinyFiles es un widget web: se abre anclado en una raiz, el
#' desplegable para cambiarla es poco visible y sus textos van en ingles dentro
#' de una aplicacion en castellano. La consecuencia practica es que parece que
...
#' ademas el pipeline necesita un directorio, no una lista de ficheros subidos.
#'
#' Advertencia: el dialogo BLOQUEA el proceso de R mientras esta abierto, de
#' modo que la aplicacion queda congelada hasta que se elige o se cancela. En
#' una aplicacion local de un solo usuario es aceptable; en un despliegue
#' compartido no lo seria, y por eso solo se ofrece cuando se detecta que la
#' ejecucion es local.
...
#' tcltk queda descartado a proposito: en macOS necesita XQuartz, que no viene
#' instalado, y ofrecer una opcion que falla es peor que no ofrecerla.
...
#'
#' @param titulo texto de la ventana.
...
#'   es un error: es la respuesta mas comun despues de elegir, y tratarla como
...
elegir_directorio_nativo <- function(titulo = "Selecciona una carpeta",
  ...
  salida <- tryCatch({
    if (so == "Darwin") {
      # `activate` trae el dialogo al frente. Sin el se abre DETRAS de la
      # ventana de la aplicacion, y como R queda bloqueado el usuario ve una
      # aplicacion congelada sin ninguna ventana que explique por que.
      guion <- sprintf(
  ...
  # Cancelar devuelve un codigo distinto de cero (y en macOS el texto "User
...
#' Valida una ruta escrita o pegada a mano.
#'
