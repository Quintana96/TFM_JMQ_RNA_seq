...
#' tabla no conserva ni la pertenencia gen-termino ni el ranking. Por eso los
#' runners de utils_enrich.R devuelven `obj` ademas de `table`.
#'
...
#' aristas. Para la memoria interesa mas la descarga en PNG a 300 ppp que la
#' interactividad.
...
ENRICH_PLOT_TIPOS <- list(
  ...
  emap  = list(etiqueta = "Mapa de terminos",  enfoques = c("ora", "gsea")),
  upset = list(etiqueta = "Solapamiento (upset)", enfoques = c("ora", "gsea")),
  ridge = list(etiqueta = "Distribucion (ridge)", enfoques = "gsea")
)
...
# ── Similitud entre terminos ────────────────────────────────────────────────

#' emapplot y treeplot necesitan la matriz de similitud, que pairwise_termsim()
#' calcula y guarda en el propio objeto. No es gratis —compara cada termino con
#' todos los demas—, asi que se limita a los terminos que se van a dibujar.
#'
#' Devuelve el objeto anotado, o una condicion si no se pudo calcular.
enrich_con_similitud <- function(obj, top_n = 30) {
  if (!requireNamespace("enrichplot", quietly = TRUE)) {
    stop("enrichplot no esta instalado.")
  }
  tryCatch(
    ...
    error = function(e) {
      ...
      # los genes, que no requiere nada mas. Si aun asi falla, el mensaje real
      # es mas util que un grafico vacio.
      stop(paste0("No se pudo calcular la similitud entre terminos: ",
                  conditionMessage(e)))
    }
  )
}
...
#' @param top_n numero de categorias a mostrar.
...
#'   el grafico, simplemente deja los genes sin color.
...
enrich_make_network_plot <- function(obj, tipo, top_n = 15, fold_change = NULL,
  ...
  if (!n_terminos) stop("El enriquecimiento no devolvio terminos.")
  # as.numeric() y no as.integer(): ridgeplot() de enrichplot 1.30 comprueba el
  # tipo de showCategory de una forma que da "integer" por invalido, avisa de que
  # "should be a number of pathways" y muere despues con "objeto 'selected' no
  ...
  con_avisos_de_enrichplot(switch(
    tipo,
    # barplot es un metodo S3 sobre graphics::barplot: se registra al cargar el
    ...
    cnet  = {
      ...
      #
      # El tope de 5 categorias no es arbitrario: un termino GO de bacteria
      # arrastra facilmente 200 genes, y con ocho terminos el grafico deja de
      ...
      # que es lo unico que el grafico tiene que comunicar.
...
#' Texto de ayuda de cada tipo, para que el grafico no haya que interpretarlo
#' de memoria. Se muestra sobre el grafico en la interfaz.
enrich_plot_ayuda <- function(tipo) {
  ...
  switch(
    ...
    cnet  = paste("Cada termino se une a los genes que lo sostienen. Sirve para",
                  ...
                  "por su cambio. Se limita a los 5 terminos mas significativos",
                  "aunque el deslizador pida mas: por encima de ahi el grafico",
                  "deja de leerse."),
    emap  = paste("Cada nodo es un termino y cada arista la fraccion de genes",
                  "que comparten. Los grupos que se forman son familias de",
                  "terminos redundantes: leerlos como hallazgos independientes",
                  "es el error mas comun al interpretar un enriquecimiento."),
    upset = paste("Cuantifica lo que la red sugiere: cuantos genes son",
                  "exclusivos de un termino y cuantos caen en intersecciones.",
...
#'
#' pathview no devuelve el grafico: descarga el KGML y el PNG de la ruta desde
#' KEGG y ESCRIBE los ficheros en el directorio de trabajo. Por eso aqui se
#' cambia de directorio a uno temporal y se restaura siempre, y por eso la
#' funcion devuelve una ruta de fichero y no un objeto.
#'
#' Requiere ademas que pathview este ADJUNTO, no solo cargado: internamente
...
#' @param species codigo KEGG del organismo ("eco", "hsa", ...).
...
enrich_pathview_png <- function(pathway_id, fold_change, species = "eco",
  ...
  if (!requireNamespace("pathview", quietly = TRUE)) {
    return(list(path = NULL, error = "pathview no esta instalado."))
  }
  ...
  sufijo <- "tfm"
  out <- tryCatch({
    # attachNamespace en lugar de library() para no ensuciar la sesion de Shiny
    # mas de lo necesario, pero con el mismo efecto sobre data(bods).
