...
#' Evaluación por bootstrap de la replicabilidad de un análisis diferencial.
#'
#' Por qué existe (docs/REVISION_ESTADISTICA.md, B8): el estudio de
...
#' replicabilidad es pobre (mediana < 0,5) en casi todos, que el mínimo
#' aceptable está en 5-7, y que con 10 la mayoria supera precisión 0,9. Su
#' recomendación es operativa: estimar por BOOTSTRAP la replicabilidad de tus
#' propios resultados antes de publicarlos, con Spearman > 0,9 como señal de
#' precisión alta y < 0,8 como aviso de probables falsos positivos.
#'
...
#' lo que el número de remuestreos es configurable.

#' Umbrales de interpretación del estudio de cohortes pequeñas.
REPLICABILITY_HIGH <- 0.9
REPLICABILITY_LOW  <- 0.8

#' Remuestrea muestras dentro de cada grupo, conservando el diseño.
#'
#' El remuestreo es ESTRATIFICADO por condición: un bootstrap ingenuo sobre
...
#' @param batch nombre de la columna de batch si está en el modelo. Cuando la
#'   hay, la estratificacion es por condición x batch: si solo se estratifica
#'   por condición, un remuestreo puede dejar un batch con una sola muestra o
#'   colineal con la condición, y ese remuestreo falla. Como los fallos solo se
#'   cuentan, la estimación acaba apoyada en los remuestreos "fáciles" y sale
#'   optimista.
bootstrap_sample_indices <- function(meta, min_per_group = 2L, batch = NULL) {
  ...
  # Con estratos muy pequeños (frecuente en condición x batch) exigir 2 muestras
  # distintas por estrato es imposible; el mínimo se aplica entonces por
  # CONDICIÓN, que es lo que el contraste necesita de verdad.
  ...
  for (lv in unique(estrato)) {
    ...
    for (attempt in 1:20) {
      ...
      # sample(pos, 1) interpreta `pos` como 1:pos y devuelve un índice
      # cualquiera en ese rango en lugar de esa muestra. Los estratos de tamaño
      # 1 son frecuentes al cruzar condición x batch.
  ...
  if (usar_batch) {
    # Comprobación final a nivel de condición: cada grupo del contraste tiene
...
#'   - `spearman`: correlación de rangos del estadístico sobre los genes comunes.
...
#' Interpretación de la replicabilidad según los umbrales publicados.
interpret_replicability <- function(median_spearman) {
  if (is.na(median_spearman)) {
    return(list(level = "desconocida", label = "No se ha podido estimar",
                detail = "No hay remuestreos válidos suficientes."))
  }
  if (median_spearman >= REPLICABILITY_HIGH) {
    return(list(level = "alta", label = "Replicabilidad alta",
      detail = paste0("La correlación de Spearman mediana entre remuestreos es ",
  ...
  if (median_spearman < REPLICABILITY_LOW) {
    return(list(level = "baja", label = "Replicabilidad baja",
      detail = paste0("La correlación de Spearman mediana es ",
  ...
  list(level = "intermedia", label = "Replicabilidad intermedia",
    detail = paste0("La correlación de Spearman mediana es ",
                    ...
                    "genes del top de la lista son más fiables que la cola."))
}

#' Panel de replicabilidad por bootstrap.
#'
#' @param counts,meta matriz y metadatos ya alineados y prefiltrados
#' @param n_boot número de remuestreos (20-50 es abordable)
#' @param progress función opcional f(i, n) para informar del avance
...
bootstrap_replicability <- function(counts, meta, method = "DESeq2",
  ...
  # Con menos de 3 muestras por grupo el bootstrap es DEGENERADO: el único
  # remuestreo que conserva 2 muestras distintas es el original, así que la
  # correlación sale 1 y se reportaria "replicabilidad alta" cuando en realidad
  # no se ha medido nada. Es justo el tipo de conclusión engañosa que este panel
  # existe para evitar, así que se rechaza en lugar de devolver un número bonito.
  ...
  if (min_group < 3) {
    return(fail(paste0(
      "El grupo más pequeño tiene ", min_group, " muestras. Con menos de 3 por ",
      "grupo el remuestreo solo puede reproducir las mismas muestras, de modo ",
      "que la correlación saldría 1 sin haber medido nada. Este diagnóstico ",
      "necesita al menos 3 replicas por grupo (la literatura situa el mínimo ",
      "aceptable para replicabilidad real en 5-7).")))
  }

  # El análisis de referencia: el mismo que se remuestrea, sin encogido (no
  # aporta nada a la correlación de rangos y cuesta tiempo x n_boot).
  ...
  if (is.null(ref) || is.null(ref$table)) {
    return(fail(paste0("El análisis de referencia fallo: ",
                       ref$error %||% "error desconocido")))
  }

  # with_seed y no set.seed a secas: fijar la semilla sin restaurarla altera el
  # RNG del resto de la sesión, que es justo lo que global.R declara que NO pasa.
  # Los índices se generan de una vez para que el resultado sea reproducible
  # incluso si la ejecución se paraleliza.
  ...
  if (!any(valid)) {
    return(fail(paste0("No se pueden generar remuestreos con al menos 2 muestras ",
                       "por grupo. Hacen falta más replicas.")))
  }
  ...
  one_boot <- function(idx) {
    ...
    # DESeq2 exige nombres únicos. Hay que reponer también los rownames: al
  ...
  summary <- data.frame(
    metrica = c("Spearman del estadístico", "Jaccard del top-N",
