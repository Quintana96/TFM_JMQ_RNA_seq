# Graficos de red y distribucion del enriquecimiento (R/utils_enrich_plots.R).
#
# Se apoyan en la API de enrichplot, que ha cambiado entre versiones de forma no
# retrocompatible, asi que estos tests valen sobre todo como deteccion temprana:
# si una actualizacion mueve un argumento, fallan aqui y no en una captura de la
# memoria.
...
# minimo con datos sinteticos para no depender ni de la red ni de una ejecucion
# previa del pipeline.
test_that("los cinco graficos se dibujan sobre un enriquecimiento real", {
  ...
  skip_if(length(tca) < 10, "no se pudieron recuperar los genes del termino")
  ...
  skip_if(is.null(ego) || !nrow(as.data.frame(ego)), "sin terminos enriquecidos")
...
test_that("showCategory llega a enrichplot como doble, no como entero", {
  # ridgeplot() de enrichplot 1.30 da por invalido un showCategory de tipo
  # "integer": avisa de que "should be a number of pathways" y muere despues con
  # "objeto 'selected' no encontrado". El error no menciona el tipo, asi que sin
  ...
  # Un doble de prueba con su propio metodo de as.data.frame: mockear el de base
  ...
  # Y el recorte al numero de terminos disponibles tampoco puede reintroducirlo.
  enrich_make_network_plot(falso, "ridge", top_n = 500L)
  expect_false(is.integer(capturado))
  expect_equal(capturado, 20)
})

# ── Traduccion de identificadores para KEGG ─────────────────────────────────
...
test_that("KEGG traduce a ENTREZID cuando se le pide ncbi-geneid", {
  ...
  # Los ENTREZID de E. coli son numericos; los simbolos no.
  expect_true(all(grepl("^[0-9]+$", recibido$gene)))
  expect_true(all(grepl("^[0-9]+$", recibido$universe)))
  # El fondo tiene que traducirse tambien: una lista en un espacio y un universo
...
test_that("sin from_keytype, KEGG pasa los genes tal cual", {
  ...
  # keyType "kegg" espera los b-numbers: no hay traduccion que hacer.
  run_enrichment_kegg(c("b0001", "b0002"), organism = "eco", keyType = "kegg")
  expect_equal(recibido, c("b0001", "b0002"))
})

test_that("una traduccion imposible se explica en vez de devolver una tabla vacia", {
