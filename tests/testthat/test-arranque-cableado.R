...
#' Dos guardas sobre la estructura de la app, no sobre su estadistica:
#'
#'   1. La raiz de la aplicacion. `runApp()` solo fija el working directory
#'      cuando recibe una RUTA; app.R le pasa un objeto app, asi que con
#'      `Rscript app.R` desde otro directorio el wd seguia siendo el del usuario.
...
#'      no lo declara ningun control es codigo muerto silencioso: no falla, no
...
#' arrancaria el servidor). Se evalua solo la expresion que la define.
...
test_that("con `Rscript app.R` desde otro directorio se resuelve la raiz real", {
  resolver <- cargar_resolve_app_dir()
  skip_if(is.null(resolver), "resolve_app_dir() no esta en app.R")
  ...
  # Y con esa raiz, las rutas derivadas caen dentro del proyecto y no del wd.
...
#' Ids de input que la aplicacion lee pero que no crea ningun control de la
...
test_that("no hay observers escuchando inputs que ningun control declara", {
  ...
  # no existia en ninguna parte de la interfaz.
